...
// ====== USER WIRING ======
static constexpr int IMU_SCK  = 13;
static constexpr int IMU_MISO = 12;
static constexpr int IMU_MOSI = 11;
static constexpr int IMU_CS   = 10;

// ====== SPI CONFIG ======
static constexpr uint32_t SPI_HZ = 10000000; // BMI160 supports up to 10 MHz
...
static constexpr uint8_t REG_ACC_DATA_START = 0x12; // ax..az 6 bytes

// ====== LOW-LEVEL HELPERS ======
static inline void csLow()  { digitalWrite(IMU_CS, LOW); }
static inline void csHigh() { digitalWrite(IMU_CS, HIGH); }

static uint8_t rd8(uint8_t reg)
{
  imuSpi.beginTransaction(SPISettings(SPI_HZ, MSBFIRST, SPI_MODE));
  csLow();
...
bool imu_init()
{
  ...
  delay(5); // accel start-up time (datasheet typical)
...
{
  // Burst gyro (6) + accel (6). Two bursts keeps it simple and fast.
  uint8_t g[6], a[6];
  burstRead(REG_GYR_DATA_START, g, sizeof(g));
  burstRead(REG_ACC_DATA_START, a, sizeof(a));
